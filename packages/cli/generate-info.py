...
def process_file_node(
    ...
    if old_node and old_node.get("description"):
        ...
        new_node["description"] = get_default_description(name, "file")
...
def process_directory(
    ...
    for entry in entries:
        ...
        if os.path.isfile(full_path):
            ...
            if old_node:
                # 保留旧的元数据
                new_node["description"] = old_node.get(
                    "description",
                    get_default_description(entry, "folder"),
                )
                if old_node.get("hidden") is not None:
                    new_node["hidden"] = old_node["hidden"]
                if old_node.get("redirect"):
                ...
                if mount_source:
                    new_node["mount_source"] = mount_source
                if old_node.get("hold"):
                    new_node["hold"] = old_node["hold"]
                ...
                new_node["description"] = get_default_description(entry, "folder")
                new_node["hidden"] = True
            else:
                new_node["description"] = get_default_description(entry, "folder")
    ...
    if old_info:
        for name, old_node in old_info["children"].items():
            if name not in new_info["children"]:
                # 检查是否为虚拟节点（有 redirect 或 mount_source 字段）
                if old_node.get("redirect") or get_mount_source(old_node):
                    preserved_node = dict(old_node)
                    node_type = preserved_node.get("type", "file")
                    if not preserved_node.get("description"):
                        preserved_node["description"] = get_default_description(
                            name,
                            "folder" if node_type == "folder" else "file",
                        )
                    new_info["children"][name] = preserved_node
    ...
    if not new_info["self"].get("description"):
        new_info["self"]["description"] = get_default_description(
            os.path.basename(dir_path),
            "self",
        )

    if new_info["self"].get("hidden") is None and is_hidden_by_default(
        os.path.basename(dir_path)
    ):
        new_info["self"]["hidden"] = True
